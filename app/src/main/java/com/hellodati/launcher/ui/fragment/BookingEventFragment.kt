...
import com.hellodati.launcher.databinding.FragmentBookingEventBinding
...
class BookingEventFragment : Fragment() {
    // TODO: Rename and change types of parameters
    private var param1: String? = null
    private var param2: String? = null
    private lateinit var binding: FragmentBookingEventBinding
    private lateinit var bindingEventFragment: FragmentEventBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentBookingEventBinding.inflate(inflater, container, false)
        bindingEventFragment = FragmentEventBinding.inflate(inflater, container, false)
        GlobalScope.launch(Dispatchers.Main)  {
            ...
            val time = LocalDateTime.parse(itemReceived[itemPosition]!!.dateto.toString(), formatter)
            val date = LocalDateTime.parse(itemReceived[itemPosition]!!.datefrom.toString(), formatter)
            val formattedTime = time.format(outputFormatter2)
            val formattedDate = date.format(outputFormatter3)
            binding.hourTxt.text = formattedTime
        ...
        val datePicker =
            DatePickerDialog.OnDateSetListener { view, year, monthOfYear, dayOfMonth ->
                ...
                if (isTodayOrFutureDate(selectedDate)) {
                    ...
                    Toast.makeText(
                        context,
                        getString(R.string.no_booking_error_date1) + "" + getString(R.string.no_booking_error_date2),
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
                ...
                GlobalScope.launch {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        val createReservationInput = CreateReservationInput(
                            date = "$date $time",
                            itemId = Optional.present(itemReceived[itemPosition]!!.id),
                            numberOfPerson = nbrPerson.toDouble(),
                            reservationType = ReservationTypeEnum.events,
                            residenceId = ResidenceClient(binding.root.context).getResidenceId()
                                .toString()
                        )
